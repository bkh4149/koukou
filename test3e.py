    ...
    while True:
        ...
        for event in pygame.event.get():  # イベントを取得
            if event.type == QUIT:        # 閉じるボタンが押されたら終了
                ...
                if  (msx > 5 or msx <0 or msy <0 or msy >2) or map[msy][msx]==3 or map[msy][msx]==2 :#戻す
                #if map[msy][msx]==3 or map[msy][msx]==2 or (msx > 5 or msx <0 or msy <0 or msy >2):#これだとindex out of rangeエラー
                #     falthyとかtruthyの話、
                    msx = ox
                    msy = oy 
        if isBreak:
